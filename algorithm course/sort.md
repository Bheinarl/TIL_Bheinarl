...
- 주어진 리스트 중에서 최소값을 찾는다.   
  ![선택정렬과정_1](./images/선택정렬과정_1.png)

- 그 값을 리스트의 맨 앞에 위치한 값과 교환한다.   
  ![선택정렬과정_2](./images/선택정렬과정_2.png)

- 맨 처음 위치를 제외한 나머지 리스트를 대상으로 위의 과정을 반복한다.   
  - 미정렬 원소가 하나 남은 상황에서는 마지막 원소가 가장 큰 값을 갖게 되므로, 실행을 종료하고 선택 정렬을 완료한다.
...
```python
def selection_sort(a,N):
	for i in range(N-1): # 기준위치 (구간 시작)
		min_idx = i # 기준 위치를 최솟값 위치로 가정
		for j in range(i+1,N):
			if a[min_idx] > a[j]:
				min_idx = j # 최솟값 찾고 저장
		a[i], a[min_idx] = a[min_idx], a[i] # 기준위치 원소와 최솟값 원소 위치 변경
```
